...
class LeadStatusService(private val client: HttpClient) {
    ...
    suspend fun getAll(): List<LeadStatus> {
        val dtos: List<LeadStatusDto> = client.get("${ApiClient.BASE_URL}/lead-statuses") {
    ...
    suspend fun create(name: String, color: String): LeadStatus {
        val dto: LeadStatusDto = client.post("${ApiClient.BASE_URL}/lead-statuses") {
            header("Authorization", "Bearer ${SessionManager.getToken()}")
    ...
    suspend fun delete(id: String) {
        client.delete("${ApiClient.BASE_URL}/lead-statuses/$id") {
            header("Authorization", "Bearer ${SessionManager.getToken()}")
        }
    }
}
